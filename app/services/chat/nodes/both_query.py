"""
DB와 Vector 검색 동시 실행 노드 (both_query)
"""
...
def both_query(state: ChatbotState) -> ChatbotState:
    ...
    """
    try:
        logger.info("DB와 Vector 검색 동시 실행 시작")
        ...
        logger.info("DB와 Vector 검색 동시 실행 완료")
        return state
        
    except Exception as e:
        logger.error(f"DB와 Vector 검색 동시 실행 실패: {str(e)}", exc_info=True)
        # 부분 실패 시에도 기본값 설정
