"""
하이브리드 PII 탐지 (KoELECTRA NER + 정규식)
"""
import re
...
class HybridPIIDetector:
    ...
    def merge_entities(self, ner_entities: List[Dict], regex_entities: List[Dict]) -> List[Dict]:
        ...
        for ner_entity in ner_entities:
            ...
            for existing in merged:
                if is_overlapping(ner_entity, existing):
                    overlapping = True
                    break
    ...
    def _propagate_known_names(self, text: str, entities: List[Dict], context_names: Set[str] = None) -> List[Dict]:
        ...
        def _is_overlapping_local(start, end, existing_entities):
        ...
        for term in search_terms:
            for match in re.finditer(re.escape(term), text):
                ...
                if not _is_overlapping_local(start, end, entities) and not _is_overlapping_local(start, end, propagated_entities):
