"""
통합 KoELECTRA 기반 PII 탐지기
(기존 detectors/dl_detector.py + audio_masking.py의 KoELECTRAPIIDetector 병합)
"""
import torch
...
class KoELECTRAPIIDetector:
    """학습된 KoELECTRA NER 모델로 PII 탐지 (문서/오디오 공통)"""

    def __init__(self, model_path: str, confidence_thresholds: Dict[str, float] = None):
        logger.info(f"KoELECTRA PII 탐지 모델 로드: {model_path}")
        ...
        logger.info(f"KoELECTRA 모델 로드 완료 (디바이스: {self.device})")
